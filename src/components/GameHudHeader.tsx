...
export default function GameHudHeader() {
  ...
  // Slide indicator to active nav item
  useEffect(() => {
    if (!navRef.current || !indicatorRef.current) return
    const activeBtn = navRef.current.querySelector(`[data-path="${location.pathname}"]`) as HTMLElement
    if (!activeBtn) {
    ...
    <header className="w-full bg-gradient-to-b from-[#1a1a2e] to-[#0d0d1a] relative sticky top-0 z-50">
      ...
      <div className="max-w-[1200px] mx-auto h-[72px] px-5 flex items-center justify-between relative z-10">
        ...
        <nav ref={navRef} className="hidden md:flex items-center gap-1 relative">
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`hud-btn font-rajdhani relative z-10 ${isActive ? '!text-teal !border-teal/40' : ''}`}
              >
                {item.label}
              </button>
            )
          ...
          <div
            ref={indicatorRef}
            className="absolute bottom-0 h-[2px] bg-teal rounded-full pointer-events-none"
